...
class JobExtractorService:
    ...
    def _is_potentially_complex(self, text: str, links: List[str]) -> bool:
        if len(links) > 1:
    ...
    def process_messages_in_batches(self, messages: List[Dict]) -> List[Job]:
        ...
        self.logger.info("Starting hybrid extraction process...")
        ...
        if complex_job_texts:
            ...
            for llm_data in llm_results:
                job_obj = self._create_job_object(llm_data)
                if job_obj:
